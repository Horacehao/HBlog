...
var pBlogApp = {},
    app = angular.module("pBlogApp", ["chieffancypants.loadingBar", "ngAnimate", "ngRoute", "ngSanitize", "pBlogApp.filters", "pBlogApp.services", "pBlogApp.directives"]).config(function($routeProvider, $locationProvider) {
        $routeProvider.when("/index", {
            ...
            $routeProvider.when("/manage", {
                templateUrl: "manage/article/list",
                controller: AboutController
            }),
...
function initializeAbout() {
    $(".menu_bar ul li").each(function(index) {
        ...
        5 == index ? $(this).addClass("active") : $(this).removeClass("active")
    }),
...
function initializeManageArticle() {
    $(".menu_bar ul li").each(function(index) {
        console.log("index:"+index)
        5 == index ? $(this).addClass("active") : $(this).removeClass("active")
    }),
        $(document).attr("title", "管理 | Horaec的个人博客")
}
var ManageArticleController = function($scope, $interval, $http, $timeout) {
    initializeManageArticle(),
        initializeLoad($scope, $timeout)
};
...
function initializeArchives() {
    $(".menu_bar ul li").each(function(index) {
        1 == index ? $(this).addClass("active") : $(this).removeClass("active")
    }),
...
var ArchivesController = function($scope, $interval, $http, $timeout) {
    $scope.fetchArchivesList = function() {
        $http.get("/archivesList.json").success(function(data) {
...
var ArchivesTimeStampController = function($scope, $interval, $http, $timeout, $routeParams) {
    ...
    $scope.fetchArchivesTimeStampList = function() {
        $http.get("/archives/" + timeStamp + ".json").success(function(data) {
            $scope.archivesList = data.archivesVOList,
...
function initializeArticleContent(content_md) {
    var $contentBody = $(".content-body"),
        contentHTML = marked(content_md),
        $tempHTML = $("<div></div>").append(contentHTML),
        counter = 0;
    ...
    $(".content-body h2").each(function(index) {
        ...
        topArray[index] = distance,
            navHTML += 0 == index ? "<li onclick='scrollArticle(" + distance + ")' id='nav-section-" + index + "'  class='active'>" + $(this).text() + "</li>": "<li onclick='scrollArticle(" + distance + ")' id='nav-section-" + index + "'>" + $(this).text() + "</li>"
    }),
        navHTML += "</ul>",
        $navList.html(navHTML),
        $(window).scroll(function() {
            var scrollDistance = $("body").scrollTop() + 30;
            scrollDistance > 165 ? $(".content-nav").addClass("fixed") : 165 >= scrollDistance && $(".content-nav").removeClass("fixed");
            for (var i = 0; i < topArray.length; i++) if (0 == i) {
                if (scrollDistance <= topArray[i + 1]) {
                    removeActiveClass(topArray),
                        $("#nav-section-0").addClass("active");
                    break
                }
            } else if (scrollDistance >= topArray[i] && scrollDistance <= topArray[i + 1]) {
                removeActiveClass(topArray),
...
function removeActiveClass(topArray) {
    for (var i = 0; i < topArray.length; i++) $("#nav-section-" + i).removeClass("active")
}
var ArticleController = function($scope, $interval, $http, $timeout, $routeParams) {
    ...
    $scope.fetchArticle = function() {
        $http.get("/article/" + articleSlug + ".json").success(function(data) {
...
function initializeBook(book_title) {
    $(".menu_bar ul li").each(function(index) {
        3 == index ? $(this).addClass("active") : $(this).removeClass("active")
    }),
...
var BookController = function($scope, $interval, $http, $timeout, $routeParams) {
    ...
    $scope.fetchBooksDetail = function() {
        $http.get("/book/" + bookSlug + ".json").success(function(data) {
...
function initializeCategory() {
    $(".menu_bar ul li").each(function(index) {
        2 == index ? $(this).addClass("active") : $(this).removeClass("active")
    }),
...
var CategoryController = function($scope, $interval, $http, $timeout) {
    $scope.fetchCategoryList = function() {
        $http.get("/categoryList.json").success(function(data) {
...
var CategorySlugController = function($scope, $interval, $http, $timeout, $routeParams) {
    ...
    $scope.fetchCategorySlugList = function() {
        $http.get("/category/" + categorySlug + ".json").success(function(data) {
            $scope.categoryList = data.categoryInfoVOList,
                $scope.articleList = data.articleInfoVOList
        }).error(function(err) {
            console.log(err)
...
function initializeIndex(content_md) {
    var $contentBody = $(".content-body");
    $(".menu_bar ul li").each(function(index) {
        0 == index ? $(this).addClass("active") : $(this).removeClass("active")
    }),
...
function loadSlider() {
    var $container = $(".scroll-area"),
        $content = $(".index-article-list"),
        $slideArea = $(".scroll-vertical"),
        $bar = $(".scroll-bar");
    $container.slider({
        container: $container,
        content: $content,
...
var IndexController = function($scope, $interval, $http, $timeout) {
    $scope.fetchArticlesList = function() {
        $http.get("/topTenArticle.json").success(function(data) {
...
var ReadController = function($scope, $interval, $http, $timeout) {
    $scope.fetchBooksList = function() {
        $http.get("/book/list.json").success(function(data) {
...
function initializeRecord() {
    $(".menu_bar ul li").each(function(index) {
        4 == index ? $(this).addClass("active") : $(this).removeClass("active")
    }),
...
var RecordController = function($scope, $interval, $http, $timeout, $routeParams) {
    $scope.fetchRecordList = function() {
        $http.get("/record.json").success(function(data) {
            $scope.recordInfoList = data.recordInfoList;
            for (var i = 0; i < data.recordInfoList.length; i++) $scope.recordInfoList[i].content = marked(data.recordInfoList[i].content)
        })
    },
        initializeRecord(),
        initializeLoad($scope, $timeout),
